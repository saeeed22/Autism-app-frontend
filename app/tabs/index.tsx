...
//       <Text className="text-2xl font-bold text-gray-800">Saeeeeeed!</Text>
//     </View>
...
// // Define props for the QuickToolButton
...
// const QuickToolButton: React.FC<QuickToolButtonProps> = ({ iconSource, label, onPress }) => {
...
//               <Image
//                 source={require('../../assets/images/recording_icon.png')}
...
//                   className="bg-white rounded-full py-3 px-8 mt-4 self-start"
//                 >
...
//             <View className="flex-row items-center">
//               <Image
//                 source={require('../../assets/images/insights_icon.png')}
//                 className="w-20 h-20 mr-4"
//                 resizeMode="contain"
//               />
//               <View className="flex-1">
...
//                 className="flex-1 rounded-3xl py-4 px-4 shadow-lg shadow-blue-400/20 overflow-hidden items-center"
//             >
//                 <LinearGradient colors={figmaBlueGradient} style={StyleSheet.absoluteFillObject} />
//                 <Image
//                     source={require('../../assets/images/consult_icon.png')}
//                     className="w-14 h-14"
//                     resizeMode="contain"
//                 />
...
//             </TouchableOpacity>
//             {/* ---------------------------------- */}
//             <TouchableOpacity className="flex-1 rounded-3xl py-4 px-4 shadow-lg shadow-blue-400/20 overflow-hidden items-center">
//                 <LinearGradient colors={figmaBlueGradient} style={StyleSheet.absoluteFillObject} />
//                 <Image
//                     source={require('../../assets/images/consult_icon.png')}
//                     className="w-14 h-14"
...
//                   Manage your child's behaviors, past stories, and learning activities
//               </Text>
//               <View className="flex-row gap-4">
//                   <QuickToolButton
//                     iconSource={require('../../assets/images/quicktool_1.png')}
//                     label={"Update\nBehaviours"}
//                     onPress={() => {  }}
//                   />
//                   <QuickToolButton
//                     iconSource={require('../../assets/images/quicktool_2.png')}
//                     label="Story History"
//                     onPress={() => { }}
//                   />
//                   <QuickToolButton
//                     iconSource={require('../../assets/images/quicktool_3.png')}
...
//               </View>
//               <Text className="text-gray-600 text-sm mb-5">
//                   See how consistently you've shared stories and track your child's insight progress
//               </Text>
...
const DUMMY_DOCTORS: Doctor[] = [
  { id: 'doc1', name: 'Dr. Kareem Bangash', specialty: 'Therapist', location: 'Gulistan-e-Johar, block-7', avatar: require('../../assets/images/doctor_1.png'), rating: 5, reviews: 1872, patients: '2,000+', experience: '10+', about: 'Dr. Kareem Bangash, a compassionate therapist, offers personalized mental health support at Serenity Wellness Clinic in Shah Faisal, Karachi.', workingTime: 'Monday - Friday, 9.00 AM to 6.00 PM', education: 'Earned a Doctorate in Clinical Psychology from the University of Washington, with advanced training in cognitive-behavioral and trauma-focused therapies.', areaOfFocus: ['Anxiety', 'Depression', 'Relationship challenges', 'Stress management'], },
  { id: 'doc2', name: 'Dr. Raheem Khan', specialty: 'Therapist', location: 'Gulistan-e-Johar, block-7', avatar: require('../../assets/images/doctor_2.png'), rating: 4.8, reviews: 980, patients: '1,500+', experience: '8+', about: 'Dr. Raheem Khan specializes in adolescent therapy and family counseling, focusing on creating a safe and supportive environment for personal growth and healing.', workingTime: 'Tuesday - Saturday, 10.00 AM to 7.00 PM', education: 'M.Phil in Clinical Psychology from Bahria University, with a certification in family systems therapy.', areaOfFocus: ['Adolescent issues', 'Family counseling', 'Academic stress', 'Self-esteem'], },
  { id: 'doc3', name: 'Dr. Fatima Ali', specialty: 'Psychiatrist', location: 'Clifton, block-2', avatar: require('../../assets/images/doctor_1.png'), rating: 4.9, reviews: 1245, patients: '1,800+', experience: '12+', about: 'Dr. Fatima Ali is a board-certified psychiatrist with extensive experience in mood disorders and psychopharmacology, dedicated to providing comprehensive care.', workingTime: 'Monday - Thursday, 8.00 AM to 4.00 PM', education: 'Completed her residency in Psychiatry at Aga Khan University Hospital, followed by a fellowship in child and adolescent psychiatry.', areaOfFocus: ['Mood disorders', 'Anxiety disorders', 'ADHD', 'Psychopharmacology'], },
];

const DUMMY_APPOINTMENTS: Appointment[] = [
  { id: 'app1', doctorId: 'doc1', datetime: new Date('2025-06-23T11:00:00'), status: 'upcoming' },
  { id: 'app2', doctorId: 'doc2', datetime: new Date('2025-06-23T11:00:00'), status: 'upcoming' },
  { id: 'app3', doctorId: 'doc3', datetime: new Date('2024-05-10T14:30:00'), status: 'completed' },
];
      ...
      <View className="px-5 pt-0">
        {doctors.map(doc => (
          <TouchableOpacity key={doc.id} onPress={() => onSelectDoctor(doc)} className="bg-white rounded-xl p-4 mb-4 shadow-md flex-row items-center">
            <Image source={doc.avatar} className="w-16 h-16 rounded-lg mr-4" />
            <View className="flex-1">
      ...
      <ScrollView contentContainerStyle={{ paddingBottom: 100 }}>
        <View className="p-5">
          <View className="bg-white rounded-xl p-4 shadow-md flex-row items-center">
            <Image source={doctor.avatar} className="w-20 h-20 rounded-lg mr-4" />
            <View className="flex-1">
    ...
    return (
      <View className="bg-white rounded-xl shadow-md p-4 mb-4">
        ...
        <View className="flex-row items-center border-t border-gray-100 pt-3">
          <Image source={doctor.avatar} className="w-16 h-16 rounded-lg mr-4" />
          <View className="flex-1">
...
const AppointmentFlow: React.FC<AppointmentFlowProps> = ({ onGoBackToHome }) => {
  ...
  const [doctors] = useState<Doctor[]>(DUMMY_DOCTORS);
  ...
  const handleConfirmAppointment = () => {
    ...
    if (activeAppointment) {
      setAppointments(p => p.map(a => a.id === activeAppointment.id ? { ...a, datetime: finalDateTime } : a));
    } else {
      setAppointments(p => [{ id: `app_${Date.now()}`, doctorId: activeDoctor.id, datetime: finalDateTime, status: 'upcoming' }, ...p]);
    }
    setConfirmationModalVisible(true);
  };

  const myAppointmentHandlers: MyAppointmentsHandlers = {
    handleCancelAppointment: (id: string) => setAppointments(p => p.filter(a => a.id !== id)),
  ...
  const handleSubmitRating = () => {
    console.log(`Rated Dr. ${activeDoctor?.name} with ${currentRating} stars.`);
  ...
  return (
    <SafeAreaView className="flex-1 bg-[#F0F8FF]">
      ...
      <ConfirmationModal
        ...
        onDone={() => { setConfirmationModalVisible(false); setViewMode('appointmentList'); }}
        doctor={activeDoctor}
        date={selectedDate}
        time={selectedTime}
      />
